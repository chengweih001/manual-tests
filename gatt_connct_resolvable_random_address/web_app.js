...
async function startTest() {
  ...
  try {
    ...
    logInfo(`Connect and then disconnect 10 times with 5 second delay between each.`);
    for (let i = 0; i < 10; i++) {
      logInfo(`Round ${i}`);
      ...
      logInfo(`Delay for 5 seconds before reconnect`);
      await delay(5000);      
      gattServer.disconnect();
      assertFalse(gattServer.connected, 'server.connected should be false');      
    }
